...
import 'package:weekly_report/models/report_weekly_model.dart' as rw;
...
class WeeklyServices {
  ...
  static Future<Object> getReportCrewList(Map par) async {
    try {
      final queryParameters = par;
      var url = Uri.parse(
      ...
      if (response.statusCode == SUCCESS) {
        return Success(
            response: rcrew.reportCrewModelFromJson(response.body).data);
      }
      return Failure(code: USER_INVALID_RESPONSE, errorResponse: 'No Data');
    } on HttpException {
      return Failure(
  ...
  static Future<Object> getReportWeekly(Map par) async {
    try {
      final queryParameters = par;
      var url = Uri.parse(
          'https://ptfi-lms.fmi.com/db/tessap/weekly_api/api/get_report_weekly.php?start_date=${queryParameters['startDate']}&end_date=${queryParameters['endDate']}');
      var response = await http.get(url);
      // var response =
      //     await Future.delayed(Duration(seconds: 5), () => http.get(url));
      ...
      if (response.statusCode == SUCCESS) {
        return Success(
            response: rw.reportWeeklyModelFromJson(response.body).data);
      }
      return Failure(code: USER_INVALID_RESPONSE, errorResponse: 'No Data');
    } on HttpException {
      return Failure(
  ...
  static Future<Object> getCrewReport() async {
    try {
      var url = Uri.parse(
          'https://ptfi-lms.fmi.com/db/tessap/weekly_api/api/get_crewReport.php');
      var response = await http.get(url);
      // var response =
      //     await Future.delayed(Duration(seconds: 5), () => http.get(url));

      if (response.statusCode == SUCCESS) {
        return Success(response: cr.weeklyModelFromJson(response.body).data);
      }
      return Failure(code: USER_INVALID_RESPONSE, errorResponse: 'No Data');
    } on HttpException {
      return Failure(
